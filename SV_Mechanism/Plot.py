...
for line in open("Data\\SV_microhomology.txt"):
    sv_microhomology_data.append(int(line.split("\t")[0]))
for line in open("Data\\Random_microhomology.txt"):
    random_microhomology_data.append(int(line.split("\t")[0]))
...
print("\n".join([str(x) for x in lcr_sv_hist_data]))
...
ax3.set_xticklabels(["$0$", "$10^1$", "$10^2$", "$10^3$", "$10^4$", "$10^5$"])
ax3.set_yticks([0, 1, 2, 3, 4, 5])
ax3.set_yticklabels(["0", "10", "100", "1000", "10000", "100000"])
ax3.set_xlabel("SV start distance to next LCR [bp]")
ax3.set_ylabel("SV end distance to next LCR [bp]")
ax3.set_title("LCR distance SV")
...
lcr_random_scatter_data = {}
lcr_random_type_data = {}
for line in open("Data\\Random_LCR_distance.txt"):
    lcr_random_hist_data.append(int(line.split("\t")[4]))

    sv_id = int(line.split("\t")[3])
    dist = int(line.split("\t")[4])
    type = line.rstrip().split("\t")[5]
    if dist == 0:
        dist = 1

    if sv_id in lcr_random_scatter_data:
        lcr_random_scatter_data[sv_id].append(dist)
        lcr_random_type_data[sv_id].append(type)
    else:
        lcr_random_scatter_data[sv_id] = [dist]
        lcr_random_type_data[sv_id] = [type]
print("\n".join([str(x) for x in lcr_random_hist_data]))
x = []
y = []
for key in lcr_random_scatter_data:
    if len(lcr_random_scatter_data[key]) == 2:
        x.append(lcr_random_scatter_data[key][0])
        y.append(lcr_random_scatter_data[key][1])
        """
        if lcr_random_scatter_data[key][0] == 1 and lcr_random_scatter_data[key][1] == 1:
            print(key)
            print(lcr_random_type_data[key])
        """
...
for i_x in range(0, size):
    for i_y in range(0, size):
        color = 1 - data_for_2d_hist[i_x, i_y] / np.amax(data_for_2d_hist)
        ax4.add_patch(Rectangle((i_x, i_y), 1, 1, facecolor=(color, color, color)))

ax4.set_xlim(0, 5)
ax4.set_ylim(0, 5)
ax4.set_xticks([0, 1, 2, 3, 4, 5])
ax4.set_xticklabels(["0", "10", "100", "1000", "10000", "100000"])
ax4.set_yticks([0, 1, 2, 3, 4, 5])
ax4.set_yticklabels(["0", "10", "100", "1000", "10000", "100000"])
ax4.set_xlabel("SV start distance to next LCR [bp]")
ax4.set_ylabel("SV end distance to next LCR [bp]")
ax4.set_title("LCR distance Random")
